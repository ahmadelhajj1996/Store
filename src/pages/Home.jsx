  ...
  return (
    <>
      ...
      <section className="p-4 md:p-8 ">
        ...
        <div className=" py-4 flex flex-row gap-4 overflow-x-auto pb-4 webkit-overflow-scrolling-touch scrollbar-none snap-x snap-mandatory">
          {isFetched &&
            data.map((item, index) => (
              <div
                key={item.id || index}
                className="w-[70%] xs:w-[45%] sm:w-[30%] md:w-[23%] lg:w-[18%] xl:w-[15%] shrink-0 snap-start flex flex-col gap-y-3"
              >
                <img
                  onClick={() => navigate(`/items/${item.id}`)}
                  src={item.first_variation.images_list[0] ?? " "}
                  alt={`${item.name}`}
