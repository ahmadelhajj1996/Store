...
function Product() {
  ...
  const { data = {}, isLoading } = useGet(["products", id], `products/${id}`, {
    enabled: true,
  ...
  const handleSelectAttribute = (attributeName, option) => {
    ...
    const matchedVariation = normalized?.variations?.find((variation) => {
  ...
  }, [normalized, current, selectedAttributes]);
