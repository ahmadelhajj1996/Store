...
const COLORS = {
  primary: '#06b6d4', // cyan-500 (changed from red-500)
  gradients: {
    from: '#ffffff', // white
    via: '#ecfeff', // cyan-50 (changed from red-50)
    to: '#cffafe'   // cyan-100 (changed from red-100)
  }
};
...
const Map = ({
  ...
  const generateDiagonalLines = useCallback(() => {
    return Array.from({ length: 8 }).map((_, i) => {
      ...
      return (
        <div
          key={`diag-${i}`}
          className="absolute w-px h-32 bg-gradient-to-b from-transparent via-cyan-500/10 to-transparent"
          style={{
  ...
  const generateContactInfoDecorations = useCallback(() => {
    return ['address', 'hours', 'email', 'phone'].map((item, index) => (
      <React.Fragment key={`contact-decor-${item}`}>
        <div 
          className="absolute left-0 w-4 h-0.5 bg-gradient-to-r from-transparent to-cyan-500/30 transform -rotate-45"
          style={{ top: `${25 + (index * 25)}%` }}
        />
        <div 
          className="absolute right-0 w-4 h-0.5 bg-gradient-to-l from-transparent to-cyan-500/30 transform rotate-45"
          style={{ top: `${25 + (index * 25)}%` }}
        />
      </React.Fragment>
    ));
  ...
  const floatingLines = useMemo(() => {
    return Array.from({ length: 12 }).map((_, i) => {
      ...
      return (
        <div
          key={`floating-${i}`}
          className="absolute w-px h-24 bg-gradient-to-b from-transparent via-cyan-500/10 to-transparent animate-float"
          style={{
            top: `${RandomUtils.randomInt(0, 100)}%`,
            left: `${RandomUtils.randomInt(0, 100)}%`,
            transform: `rotate(${rotation}deg)`,
  ...
  return (
    <section className="relative container py-16 overflow-hidden z-0">
  ...
  <div className="relative max-w-2xl md:max-w-4xl mx-auto text-center z-10">
          ...
          <h1 className="tracking-widest uppercase font-sans font-bold text-2xl sm:text-4xl text-center block max-w-max mx-auto">
            <span className="text-cyan-500">  موقع لين بوتيك</span>{" "}
          </h1>   
      ...
      <div className="relative mt-16 lg:mt-20 z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 gap-x-16">
          ...
          <div className="relative rounded-lg overflow-hidden shadow-2xl w-[90%] md:w-[80%] mx-auto group">
            ...
            <div className="absolute -top-4 -left-4 w-8 h-8 border border-cyan-500/10 rounded-full animate-spin-slow" />
            <div className="absolute -bottom-4 -right-4 w-8 h-8 border border-cyan-500/10 rounded-full animate-spin-slow-reverse" />
            
            {/* Decorative corner lines */}
            <div className="absolute top-2 left-2 w-6 h-px bg-gradient-to-r from-cyan-500 to-transparent transform rotate-45" />
            <div className="absolute top-2 right-2 w-6 h-px bg-gradient-to-l from-cyan-500 to-transparent transform -rotate-45" />
            <div className="absolute bottom-2 left-2 w-6 h-px bg-gradient-to-r from-transparent to-cyan-500 transform -rotate-45" />
            <div className="absolute bottom-2 right-2 w-6 h-px bg-gradient-to-l from-transparent to-cyan-500 transform rotate-45" />
            ...
            <iframe
              ...
              width="100%"
              height="480"
              className="relative z-10 rounded-lg border border-cyan-500/20 group-hover:border-cyan-500/40 transition-all duration-500"
              style={{ border: 0 }}
              allowFullScreen
              loading="lazy"
          ...
          <div className="relative px-2 md:px-6 py-8 flex flex-col gap-y-6 rounded-2xl backdrop-blur-sm border border-cyan-500/10 group hover:border-cyan-500/30 transition-all duration-500">
            ...
            <div className="absolute -top-2 -left-2 w-12 h-px bg-gradient-to-r from-cyan-500 to-transparent transform rotate-45" />
            <div className="absolute -top-2 -right-2 w-12 h-px bg-gradient-to-l from-cyan-500 to-transparent transform -rotate-45" />
            <div className="absolute -bottom-2 -left-2 w-12 h-px bg-gradient-to-r from-transparent to-cyan-500 transform -rotate-45" />
            <div className="absolute -bottom-2 -right-2 w-12 h-px bg-gradient-to-l from-transparent to-cyan-500 transform rotate-45" />
            
            {/* Animated background gradient */}
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

            {['address', 'hours' , 'phone', 'email'].map((item, index) => (
              <div 
                key={item} 
                className="relative flex items-center group/item py-3 px-2 rounded-xl hover:bg-gradient-to-r from-transparent via-cyan-500/5 to-transparent transition-all duration-300"
              >
                <div className="absolute left-0 top-1/2 w-1 h-0 bg-gradient-to-b from-cyan-500 via-cyan-500 to-transparent -translate-y-1/2 group-hover/item:h-12 transition-all duration-500" />
                
                {/* Decorative bullet */}
                <div className="w-3 h-3 border border-cyan-500/50 rounded-full group-hover/item:border-cyan-500 group-hover/item:bg-cyan-500/10 transition-all duration-300 flex-shrink-0">
                  <div className="w-1 h-1 bg-cyan-500 rounded-full mx-auto mt-1" />
                </div>
                ...
                <span className="text-black/80 text-sm md:text-base tracking-wider ml-2 group-hover/item:text-black transition-colors duration-300">
                  {item === 'address' ? 'عقربا - الحي الأوسط'  :
                  item === 'hours' ? 'من 10 صباحا الى 8 مساء' :
                  item === 'email' ? contact.email : contact.phone}
                </span>
                
                {/* Diagonal accent line on hover */}
                <div className="absolute right-4 top-1/2 w-4 h-px bg-gradient-to-l from-transparent to-cyan-500 transform -translate-y-1/2 rotate-45 opacity-0 group-hover/item:opacity-100 transition-all duration-300" />
              </div>
            ))}
            
            {/* Additional decorative elements */}
            <div className="absolute -bottom-4 left-1/4 w-1/2 h-px bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent" />
            <div className="absolute -top-4 right-1/4 w-1/2 h-px bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent transform rotate-3" />
          </div>
        </div>
      </div>
      
      {/* Section bottom decorative line */}
      <div className="relative mt-12 mx-auto w-48 h-px bg-gradient-to-r from-transparent via-cyan-500 to-transparent" />
    </section>
  );
};
