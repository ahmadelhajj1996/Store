...
useEffect(() => {
  const handleResize = () => {
    setStickyTop(window.innerWidth >= 640 ? '135px' : '122px');
  };
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          const shouldBeSticky = scrollY > initialOffset * 0.15; // 10px buffer
      ...
      <Section>
        <div
          ...
          className={`w-full transition-all duration-300 ${
            isSticky
              ? "fixed inset-x-2 z-50 bg-white shadow-lg animate-slideDown"
              : "relative"
          }`}
          style={{
        ...
        >
          <div className="grid grid-cols-2 min-[400px]:grid-cols-3 min-[500px]:grid-cols-4  gap-2 justify-center text-center items-center">
